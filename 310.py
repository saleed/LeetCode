...
class Solution(object):
    def findMinHeightTrees(self, n, edges):
        ...
        """
        if n==1:return [0]
        adj=self.edgesToAdj(edges,n)
        return self.search(adj,n)
        ...
        while not que.empty():
            ...
            for j in adj[node[0]]:
                if vis[j]==0:
                    vis[j]=1
                    que.put([j,node[1]+1])
                    retlen = max(retlen, node[1] + 1)

            if retlen>height:
                return float("inf")
        return retlen

    def search(self,adj,n):
